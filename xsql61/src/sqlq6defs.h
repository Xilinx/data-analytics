...
#ifndef XSQL61_SQLQ6DEFS_H
#define XSQL61_SQLQ6DEFS_H
#include <ap_int.h>
...
namespace SqlQ6Defs {
  ...
  const unsigned int  ProcUnitCountForBlock = 64; // 32;  // Number of proc unit operations on the Blocksize
  // Each compute unit works on CUBlockSize = 32K
  const unsigned int ProcUnitBlockSize = 32768; // 2*32768,  32*1024,  2**5 * 2**10 = 2**15
  // Maximum number of rows in a CUBlock block
  const unsigned int ProcUnitBlockMaxRows = 1024; // 16*1024,  2**14 32  Bytes in each row
  ...
  const unsigned int ReadBlockSize = 256;
