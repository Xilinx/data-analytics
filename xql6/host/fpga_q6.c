...
// device memory. 10 buffers are created to allow multiple overlapping
// operations to hide transfer latencies.
static cl_mem devInp [10];
static cl_mem devOut [10];
...
unsigned char* pInp[10];
unsigned char* pOut[10];
...
int start_fpga (const char *xclbin) {
  ...
  for (i=0; i<10; ++i) {
    ...
    devInp[i] = clCreateBuffer (gcontext, CL_MEM_READ_ONLY, XQL6_BLK_SZ, NULL, NULL);
